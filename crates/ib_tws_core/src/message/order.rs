...
pub fn decode_order_status_msg(
    ...
    let perm_id = if version >= 2 { buf.read_int()? } else { 0 };

    let parent_id = if version >= 3 { buf.read_int()? } else { 0 };
    ...
    let client_id = if version >= 5 { buf.read_int()? } else { 0 };
...
fn order_condition_read(
    buf: &mut BytesMut,
    condition_type: i32,
) -> Result<OrderCondition, io::Error> {
